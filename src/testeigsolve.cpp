...
main() {
    ...
    // receiver vector
    // std::cout << mydat.vr().rows() << " " << mydat.vr().cols() << std::endl;
    ...
    Eigen::FullPivLU<
        Eigen::Matrix<std::complex<double>, Eigen::Dynamic, Eigen::Dynamic> >
        fsolve;
    fsolve.compute(eig_vecs);
    vecf = fsolve.solve(fRHS);
    ...
    for (int idx = 0; idx < myfreq.nt(); ++idx) {
        ...
        for (int idx2 = 0; idx2 < 2 * mydat.nelem(); ++idx2) {
            if (eig_values(idx2).real() > 0) {
                ...
                //              static_cast<std::complex<double> >(vec_t[idx]));
                multfact =
                    std::exp((eig_values(idx2) - myfreq.ep()) *
                             static_cast<std::complex<double> >(vec_t[idx]));
            }
            // multfact = std::exp((eig_values(idx2) - myfreq.ep()) *
